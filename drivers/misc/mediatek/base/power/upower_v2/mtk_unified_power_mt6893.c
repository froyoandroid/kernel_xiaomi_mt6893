...
#define M6893T(range)	(1 ? range)
#define L6893T(range)	(0 ? range)
/**
 * Genearte a mask wher M6893T to L6893T are all 0b1
 * @r:	Range in the form of M6893T:L6893T
 */
#define BITMASK(r)	\
	(((unsigned int) -1 >> (31 - M6893T(r))) & ~((1U << L6893T(r)) - 1))

/**
 * Set value at M6893T:L6893T. For example, BITS(7:3, 0x5A)
 * will return a value where bit 3 to bit 7 is 0x5A
 * @r:	Range in the form of M6893T:L6893T
 */
/* BITS(M6893T:L6893T, value) => Set value at M6893T:L6893T  */
#define BITS(r, val)	((val << L6893T(r)) & BITMASK(r))
...
struct upower_tbl_info
	...
	[2] = {
		INIT_UPOWER_TBL_INFOS(UPOWER_BANK_LL,
			upower_tbl_l_30G28G),
		INIT_UPOWER_TBL_INFOS(UPOWER_BANK_L,
			upower_tbl_bl_30G28G),
		INIT_UPOWER_TBL_INFOS(UPOWER_BANK_B,
			upower_tbl_b_30G28G),
		INIT_UPOWER_TBL_INFOS(UPOWER_BANK_CLS_LL,
			upower_tbl_cluster_l_30G28G),
		INIT_UPOWER_TBL_INFOS(UPOWER_BANK_CLS_L,
			upower_tbl_cluster_bl_30G28G),
		INIT_UPOWER_TBL_INFOS(UPOWER_BANK_CLS_B,
			upower_tbl_cluster_b_30G28G),
		INIT_UPOWER_TBL_INFOS(UPOWER_BANK_CCI,
			upower_tbl_cci_30G28G),
	},
};
...
static void upower_scale_l_cap(void)
{
	...
	tbl = upower_tbl_infos[UPOWER_BANK_B].p_upower_tbl;
	...
	for (i = 0; i < NR_UPOWER_BANK; i++) {
		if ((i == UPOWER_BANK_B) || (i == UPOWER_BANK_CLS_B)) {
